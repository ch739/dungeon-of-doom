...
/**
 * Class containing the map used by the game engine. Allows for reading in ASCII art maps from a
 * file, or just using the default map.
 * 
 * You are not responsible for making this more robust - you are only required to perform error
 * checking on the extra code that you write to network the client and server.
 */
public class Map {
	// The name of the map
	private String name;
	 ...
	 */
	public Map(String filename) throws FileNotFoundException, ParseException {
		this.load("res" + File.separator + filename);

	}

	/**
	 * parses the provided map file and checks it is correct
	 * 
	 * @param filename the filename of the map
	 * @throws ParseException
	 * @throws FileNotFoundException
	 */
	private void load(String filename) throws ParseException, FileNotFoundException {
		this.setFilename(filename);
		final List<String> lines = readFile(filename);
		// Good programmers always check this...
		if (lines.size() < MINLINES) {
			throw new ParseException("a map file must contain at least three lines", lines.size());
		}
	...
	public boolean insideMap(Location location) {
		if ((location.getCol() < 0) || (location.getCol() >= getMapWidth()) || (location.getRow() < 0)
	...
	/**
	 * Reads in a file and returns a List of Strings. This makes life slightly easier.
	 * 
	 ...
	 */
	private void readMap(List<String> lines) throws ParseException, IllegalStateException {
	...
	/**
	 * Obtains the goal (number of cold to collect) from the second line of the map file
	 * 
	...
	/**
	 * A helper method to process a line in a file of the format <tag> <argument> Returns the
	 * argument, if the tag is correct
	 * 
	...
	private String getStringAfterTag(String line, String tag, int lineNum) throws ParseException {
		...
		if (firstSpace == -1) {
			throw new ParseException(tag + " not specified in file; the " + tag + " should be be preceded with \"" + tag
					+ "\"", lineNum);
		}
