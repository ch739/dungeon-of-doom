...
public class Tile {
	...
	/**
	 * Gets the tile on the item, but *does not remove it*. This allows us to see if the player
	 * already has the item, without removing it.
	 * 
	...
	/**
	 * adds a new item to the tile replacing any items currently there.
	 * @param item new item type to replace current item with
	 */
	public void addItem(char item) {
		this.item = GameItem.fromChar(item);
	}
