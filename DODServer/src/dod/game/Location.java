...
/**
 * Class to refer to locations on the map to avoid having to store the row and column separately.
 * There are also some "helper" methods to assist code readability
 */
public class Location {
	...
	public Location atOffset(int colOffset, int rowOffset) {
		final Location offsetLocation = new Location(this.col + colOffset, this.row + rowOffset);

		return offsetLocation;
	}

	/**
	 * Used to obtain the location immediately to the north, east, south or west of the current
	 * location
	 * 
	 * @param direction the compass direction to look up.
	 * @return the location immediately at the compass direction to the current location
	 */
